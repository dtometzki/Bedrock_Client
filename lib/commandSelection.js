...
async function promptForCommandSelectionInteractive(rl) {
...
export async function promptForCommandSelection(rl) {
  if (input.isTTY && typeof input.setRawMode === "function") {
    return promptForCommandSelectionInteractive(rl);
  }
  ...
  const choice = await askStyledQuestion(
    rl,
    `Befehl wählen (1-${COMMANDS.length}, Enter zum Abbrechen)`
  );

...
async function promptForModelSelectionInteractive(rl, models, currentModelId, promptLabel) {
  ...
  let hasHandledNavigationInput = false;
  ...
  return new Promise((resolve) => {
    ...
    const handleKeypress = (_, key) => {
      ...
      if (key.name === "up") {
        hasHandledNavigationInput = true;
      ...
      if (key.name === "return") {
        ...
        const shouldIgnoreInitialEnter = !hasHandledNavigationInput && elapsedMs < 250;
        if (shouldIgnoreInitialEnter) {
          return;
        }

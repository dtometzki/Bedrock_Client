...
function printChatStats(messages, activeModelId) {
  ...
  const averageCharacters = messages.length > 0 ? Math.round(totalCharacters / messages.length) : 0;
  ...
  console.log(`  Zeichen gesamt: ${totalCharacters}`);
  console.log(`  Ø Zeichen je Nachricht: ${averageCharacters}\n`);
}
...
export async function handleCommand(inputTrimmed, context) {
  ...
  if (command === "/stats") {
    printChatStats(ai.messages, ai.modelId);
    return false;
  }

...
- Lets you pick the adaptive-thinking effort level (`low`/`medium`/`high`, Opus also `max`, Opus 5 and Fable 5.1 also `xhigh`/`max`) directly in the `/model` picker for reasoning models: use the left/right arrow keys to change it
...
- Lets you pick the adaptive-thinking effort level (`low`/`medium`/`high`, Opus also `max`, Opus 5 and Fable 5.1 also `xhigh`/`max`) per reasoning model in the web GUI
...
Both interfaces use the same vault file. The GUI and terminal commands remain available even if credentials are missing, locked or expired. In the web settings, choose **Gespeicherte Schlüssel verwenden**, activate it with **Tresor-Anmeldung aktivieren** if needed, then choose **Tresor einrichten**. Setting up the vault needs no AWS request. Choose an existing **role profile**, enter a permanent Access Key ID and Secret Access Key, and choose a master password of at least 12 characters. Repeat the password to confirm it. Keys and passwords are entered in hidden fields; do not pass them in command arguments or environment variables.
...
The web settings always show **Wie möchtest du dich anmelden?** with two choices:

- **Gespeicherte Schlüssel verwenden** uses the encrypted vault and requires its master password after a process restart or inactivity lock.
- **Bestehende AWS-Anmeldung verwenden** uses AWS credentials configured outside this client. **It does not ask for a master password, including after stop and restart.** The vault keys are not used in this mode. An explanation appears before applying the change with **Bestehende AWS-Anmeldung aktivieren**; selecting an option alone does not switch modes.

The web profile picker shows all local profiles. In vault mode, profiles without `role_arn` remain visible with a **bestehende AWS-Anmeldung** label and cannot be selected as role profiles. To use a login/SSO profile such as `Admins`, choose **Bestehende AWS-Anmeldung verwenden**, select that profile and activate the change. **AWS-Profil wechseln** also remains available in this mode. This uses the existing profile session and keeps the encrypted vault unchanged; renew an expired AWS login separately (for example, `aws login --profile Admins`). The profile selection applies to the running client; use `--auth aws --profile Admins` to select it explicitly on startup.

A status line beneath the web header always shows the active source, known role (or profile), lock state and connection-check result. Role metadata remains visible after locking the running server; after a process restart the encrypted vault profile must first be unlocked before its role can be displayed. Existing AWS authentication is explicitly marked **ohne Masterpasswort**.
...
Failed connection checks show **Details zur letzten AWS-Verbindungsprüfung** in the settings: the failed step, profile, requested role for a vault role-assumption failure, known error code, and available validated HTTP status/request ID, plus suggested next steps. For role chains, the failure identifies the actual failed role, including an intermediate source role. A failure of the identity check is shown separately from a failed role assumption. No raw SDK error messages, credentials or request/response bodies are exposed. Details are shared across tabs and reloads of that server and cleared after a successful retry, locking or changing authentication. The terminal uses the same sanitized details. An access-denied code alone does not identify the denying IAM policy; inspect the failed AWS CloudTrail event, the caller's `sts:AssumeRole` permissions and the target role's trust conditions as described in [AWS role troubleshooting](https://docs.aws.amazon.com/IAM/latest/UserGuide/troubleshoot_roles.html).
...
In vault mode, the master password is required after every process restart and after 15 minutes of inactivity. Existing AWS authentication works without this master password. Typing/clicking in the client counts as activity; background status polling does not. Running AWS requests count as use, with a fresh idle period after completion. Manual locking aborts active AWS requests and discards clients, cached role credentials and the unlocked keys. Browser tabs of one server share the authentication state; reloading or closing a tab does not lock the running server. Locking revokes all browser sessions and clears the open chat views (immediately in the same browser, on the next status check in other browsers, at most five seconds while active). Separate terminal/web processes unlock independently.
...
The default browser opens automatically with the GUI (default `http://127.0.0.1:3456`); `--no-open` disables that. With an existing vault and vault authentication selected, open this normal local address and enter the master password. No URL token or private start file is needed. Bookmark this address and use the same host and port in additional windows.

After authentication, a random `HttpOnly`, `SameSite=Strict` session cookie provides access to every tab/window in the same browser profile. Another browser or private browsing profile must authenticate separately. Cookies contain no AWS credentials; session tokens are kept only in server memory and expire on a server restart, vault lock, or credential/password changes. JavaScript no longer stores the access token in `sessionStorage`; existing tab tokens are removed and exchanged where applicable. Host/Origin checks and a required custom request header protect the cookie APIs against cross-origin access and CSRF; responses are not cached. The server still binds exclusively to loopback.

For initial vault setup or **Bestehende AWS-Anmeldung verwenden**, the private startup file still proves local access. The launcher opens it automatically, or prints its path with `--no-open`. It exchanges the startup token for a shared browser cookie, so additional windows can then use the normal URL. In existing AWS authentication mode, a separate browser can use that startup file to keep the selected authentication. If a vault already exists, the normal local address also offers **Mit Tresor anmelden** with a master-password field. Submitting the correct password explicitly switches to the saved vault credentials and remembers vault mode for subsequent starts. Simply opening the page or entering a wrong password leaves the existing AWS authentication unchanged. The server-control token used by `--web-stop` remains separate from browser cookies. AWS role session tokens and their automatic refresh are unchanged.

Add `--background` to release the terminal after the web server has started successfully. The server runs independently and continues when the terminal is closed. Stop it from another terminal with `node app_aws.js --web-stop`; no process ID needs to be remembered. With multiple background servers, select one using `--web-stop --port 8080`. The client keeps a private `web-background-<port>.json` file in its configuration directory containing the local port and server access token. This file is removed on normal shutdown. Stopping authenticates to that specific server instead of signalling a saved PID, so a stale entry cannot kill an unrelated process. The stop command also works while the vault is locked. Servers started before this feature need to be stopped once using their previously printed command and then restarted. Startup failures such as an occupied port are reported before the launcher exits. `--no-open` also works in background mode: it prints the normal URL for an existing vault in vault mode, or the private browser start-file path for initial setup/existing AWS authentication. Normal `--web` stays in the foreground and ends with Ctrl+C. Background mode keeps the same loopback protection and vault locking rules. The background process starts with the vault locked; unlock it on the browser welcome screen. It does not automatically start at operating-system login.

Each newly opened tab/window starts a separate, empty conversation, including when you paste the same URL or open it from another window. Reloading keeps that window's conversation. Model, effort, system prompt, attachments, usage totals, clearing and stopping an answer belong to that conversation; two windows can stream independently. AWS authentication, vault locking and account-wide billing remain shared. A non-secret conversation identifier is kept in `sessionStorage` and sent in a request header; it never replaces the authenticated browser cookie. If browser storage is disabled, the conversation cannot be restored after a reload.

With auto-save enabled, window transcripts are stored separately in private `web-chats/<id>.json` files inside the configuration directory (`0700` directory, `0600` files). They do not overwrite the CLI's `last-session.json`. A still-open window can reload its saved transcript after a server restart and authentication; `--no-save` keeps transcripts only for the lifetime of that server. Closing a window ends its browser-side association; saved files remain until cleared or removed. **Verlauf leeren** deletes only that window's saved transcript. There is no cross-window conversation picker in this version. The server holds at most 64 window conversations per start and refuses additional ones instead of silently discarding existing chats.
...
Claude Opus 5 is available as `us.anthropic.claude-opus-5` (selection name `claude-opus-5`), alongside Opus 4.8. Start it with `node app_aws.js -m claude-opus-5 --region us-east-1`, or select it in `/model` or the web GUI after restarting. It uses adaptive thinking with `low`, `medium`, `high` (default), `xhigh`, and `max`; Temperature and Top-P overrides are omitted. The [AWS model documentation](https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-anthropic-claude-opus-5.html) lists a 1M-token context and up to 128K output tokens. Existing client history limits and the default 4,096-token output budget remain in effect. Cost estimates use the [published Anthropic rates](https://platform.claude.com/docs/en/models/opus-5/overview) of $5 input / $25 output per million tokens, including thinking tokens in output; actual AWS billing can differ.
